...
import 'package:voiture/formulaireCommande/components/my_button.dart';
import 'package:voiture/formulaireCommande/components/my_textfield.dart';
import 'package:voiture/formulaireCommande/components/square_tile.dart';
...
class _AddCarFormState extends State<AddCarForm> {
  ...
  dynamic getCategoryIndex(String category) {
  ...
  void userOrder(Map model) {
    if (selectedCategory != null && selectedCarBrand != null && isFilled ) {
      AwesomeDialog(
        ...
        desc: "Vous allez ajouter une nouvelle voiture",
        btnCancelText: "Annuler", // Modifier le texte du bouton Cancel
        btnCancelOnPress: () {},
        btnOkOnPress: () async {
          try {
            var user = Hive.box('settings').get('user');
            print("------getcategorie-VVVVVVVVVVVVV---");
            print("Données JSON ------------------------------- : $model");
            final response = await http.post(Uri.parse('$apiBaseUrl/api/postVehicle/${user['id']}/'),
                body :<String, dynamic> {
                  'marque': model['marque'],
                  'numPlaque': model['numPlaque'],
                  'typeBoiteVitesse': model['typeBoiteVitesse'],
                  'assurance': model['assurance'],
                  'numChassis': model['numChassis'],
                  //'category': getCategoryIndex(selectedCategory!),
                  'nombreCylindres' : int.tryParse(model['nombreCylindres'].toString()) ?? 0,
                },);
            print("ID USER ${user['id']}");
            print("----------------------------- ${response}");
            print("----------------------------- 'marque'");
            List vs = Hive.box('settings').get('vehicules') ?? [];
            vs.add(model);
            await Hive.box('settings').put('vehicules', vs);
      ...
      AwesomeDialog(
        ...
        desc: "Veuillez remplir tous les champs avant d'ajouter votre voiture.",
        btnOkOnPress: () {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Colors.transparent, elevation: 0,title: Text("Ajouter une voiture",style: TextStyle(color: Colors.black),),
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Veuillez entrez s\'il vous plaît les informations demandées !',
                  style: TextStyle(
                ...
                Text(
                  'Nombre de cylindre', // Texte que vous souhaitez ajouter
                  style: TextStyle(
                    color: Colors.grey[700], // Couleur du texte
                    fontSize: 16, // Taille du texte
                    fontWeight: FontWeight.bold, // Texte en gras
                ...
                const SizedBox(height: 10,),
                MyTextField(
                  controller: nbCylindreController, // Utilisation du contrôleur pour le numéro de châssis
                  hintText: 'Nombre de cylindre', // Texte d'invite pour le numéro de châssis
                ...
                MyButton(
                  onTap: (){
                    userOrder({
                      'marque' : selectedCarBrand,
                      'numPlaque' : plaqueController.text.trim(),
                      'typeBoiteVitesse' : selectedBoite ?? '',
                      'assurance' : selectedAssurance ?? '',
                      'numChassis' : chassisNumberController.text.trim(),
                      'nombreCylindres': int.tryParse(nbCylindreController.text) ?? 0,
                      //'category' : getCategoryIndex(selectedCategory!),
                    });
                  },
